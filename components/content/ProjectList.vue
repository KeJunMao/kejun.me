...
<template>
  <div v-for="(category, categoryName) in projects" :key="categoryName" class="group">
    ...
    <div :class="[category.length === 1 ? '' : '']" grid="~ cols-1 md:cols-2 lg:cols-3 xl:cols-4 gap-4 ">
      <NuxtLink v-for="project in category" :key="project.name" :to="project.link" target="_blank" rounded-xl p-4 border="~ zinc/7 hover:transparent" class="group/item transition-all hover:bg-zinc/7">
